	...
	<div class="col-md-12">
		...
		<div class="comments">
			<c:forEach var="tmp" items="${commentList }">
				<div class="comment" <c:if test="${tmp.board_num ne tmp.comment_group }">style="margin-left:100px"</c:if> >	
					<c:if test="${tmp.board_num ne tmp.comment_group }">
					...
					<form action="comment_insert.do" method="post">
						...
						<!-- 덧글 그룹 -->
						<input type="hidden" name="ref_group" value="${dto.board_num }" />
						<!-- 덧글 대상 -->
...
<script>
	...
	$(".comment_form > form, .comment form").submit(function(){
		if(!isLogin){//만일 로그인 하지 않았다면 
			...
			"/customer/signin_form.do"+
			"?url=${pageContext.request.contextPath}"+
			"/board/detail.do?board_num=${dto.board_num}";
	...
	var dropOut = function(){
		...
		if(isDelete){
			location.href = "members/delete.do?board_num=${dto.board_num}";
		}
	}
</script>
</body>
